/**
 * 深海女仆工坊（maid-atelier）整包运行时。
 *
 * 移植自 .tmp-skins/dsh-deep-whale/maid-atelier/src/client/index.ts：
 * 背景、双角色立绘、可收纳侧栏装饰与独立挂饰层。侧栏保留产品原生
 * 矢量字标；一切皮肤侧写入都登记进 PackContext，由 SkinManager 统一
 * dispose 恢复。与上游的差异：
 *  - Context 换为 dsh-skin 的 PackContext（effect 语义相同）；
 *  - module.css 不再走 bundler 注入，改为 apply 时挂 <style>、dispose 时摘除；
 *  - 素材来自 scripts/extract-presets.mjs 生成的 *.generated.ts。
 */
...
import { MAID_PALACE_DAY, MAID_PALACE_NIGHT } from '../../presets/backgrounds.generated'
import { MAID_ATELIER_CSS } from './css.generated'
import { MAID_ATELIER_TITLEBAR_BRAND } from './titlebar-brand.generated'

const SKIN_TITLE = '深海女仆工坊 · DeepSeek Harness'
const SKIN_OWNER = 'maid-atelier'
const SKIN_SYSTEM_CHROME_COLOR = '#0b193f'

/** TS DOM lib 尚无 WindowControlsOverlay 类型；运行时不支持的环境返回 undefined。 */
 ...
 * @param ctx - owning context whose effects retract every DOM and CSS write.
 */
export function applyMaidAtelier(ctx: PackContext): void {
  ...
  let titlebarOverlay: WindowControlsOverlayLike | undefined
  ...
  }, 'pack/maid-atelier: layered background and ornament')

  // 整包样式随 activation 注入（上游走 bundler 常驻注入，这里挂到
  // ownedNodes，dispose 时一并摘除，热切换不留样式残留）。
  const styleTag = document.createElement('style')
  styleTag.dataset.skinChrome = 'pack-style'
  styleTag.dataset.skinOwner = SKIN_OWNER
  styleTag.textContent = MAID_ATELIER_CSS
  ...
  titlebarOverlay = (navigator as Navigator & { windowControlsOverlay?: WindowControlsOverlayLike }).windowControlsOverlay
